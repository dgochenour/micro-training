...
BrakeCommand *
BrakeCommand_create(void)
{
    BrakeCommand* sample;
    OSAPI_Heap_allocate_struct(&sample, BrakeCommand);
    if (sample != NULL)
    {
        if (!BrakeCommand_initialize(sample))
        {
            OSAPI_Heap_free_struct(sample);
            sample = NULL;
        }
...
BrakeCommand_delete(BrakeCommand*sample)
{
    if (sample != NULL)
    {
        /* BrakeCommand_finalize() always 
        returns RTI_TRUE when called with sample != NULL */
        BrakeCommand_finalize(sample);
        OSAPI_Heap_free_struct(sample);
    }
}

/* ========================================================================= */
const char *BrakeCommandTYPENAME = "BrakeCommand";

#ifndef NDDS_STANDALONE_TYPE
DDS_TypeCode* BrakeCommand_get_typecode()
{
    static RTIBool is_initialized = RTI_FALSE;

    static DDS_TypeCode BrakeCommand_g_tc_device_id_string = DDS_INITIALIZE_STRING_TYPECODE(((DEVICE_ID_MAX_LENGTH)));

    static DDS_TypeCode_Member BrakeCommand_g_tc_members[2]=
    {
        ...
        {
            (char *)"brake_intensity",/* Member name */
            {
                1,/* Representation ID */
                DDS_BOOLEAN_FALSE,/* Is a pointer? */
                -1, /* Bitfield bits */
    ...
    static DDS_TypeCode BrakeCommand_g_tc =
    {{
            DDS_TK_STRUCT, /* Kind */
            DDS_BOOLEAN_FALSE, /* Ignored */
            -1, /*Ignored*/
            (char *)"BrakeCommand", /* Name */
            NULL, /* Ignored */      
            0, /* Ignored */
            0, /* Ignored */
            NULL, /* Ignored */
            2, /* Number of members */
            BrakeCommand_g_tc_members, /* Members */
            DDS_VM_NONE, /* Ignored */
            RTICdrTypeCodeAnnotations_INITIALIZER,
            DDS_BOOLEAN_TRUE, /* _isCopyable */
            NULL, /* _sampleAccessInfo: assigned later */
            NULL /* _typePlugin: assigned later */
        }}; /* Type code for BrakeCommand*/

    if (is_initialized)
    {
        return &BrakeCommand_g_tc;
    }

    BrakeCommand_g_tc._data._annotations._allowedDataRepresentationMask = 5;

    BrakeCommand_g_tc_members[0]._representation._typeCode = (RTICdrTypeCode *)&BrakeCommand_g_tc_device_id_string;
    BrakeCommand_g_tc_members[1]._representation._typeCode = (RTICdrTypeCode *)&DDS_g_tc_float;

    /* Initialize the values for member annotations. */
    BrakeCommand_g_tc_members[0]._annotations._defaultValue._d = RTI_XCDR_TK_STRING;
    BrakeCommand_g_tc_members[0]._annotations._defaultValue._u.string_value = (DDS_Char *) "";

    BrakeCommand_g_tc_members[1]._annotations._defaultValue._d = RTI_XCDR_TK_FLOAT;
    BrakeCommand_g_tc_members[1]._annotations._defaultValue._u.float_value = 0.0f;
    BrakeCommand_g_tc_members[1]._annotations._minValue._d = RTI_XCDR_TK_FLOAT;
    BrakeCommand_g_tc_members[1]._annotations._minValue._u.float_value = RTIXCdrFloat_MIN;
    BrakeCommand_g_tc_members[1]._annotations._maxValue._d = RTI_XCDR_TK_FLOAT;
    BrakeCommand_g_tc_members[1]._annotations._maxValue._u.float_value = RTIXCdrFloat_MAX;

    BrakeCommand_g_tc._data._sampleAccessInfo =
    BrakeCommand_get_sample_access_info();
    BrakeCommand_g_tc._data._typePlugin =
    BrakeCommand_get_type_plugin_info();    
...
RTIXCdrSampleAccessInfo *BrakeCommand_get_sample_access_info()
{
    static RTIBool is_initialized = RTI_FALSE;

    BrakeCommand *sample;

    static RTIXCdrMemberAccessInfo BrakeCommand_g_memberAccessInfos[2] =
    {RTIXCdrMemberAccessInfo_INITIALIZER};

    static RTIXCdrSampleAccessInfo BrakeCommand_g_sampleAccessInfo = 
    RTIXCdrSampleAccessInfo_INITIALIZER;

    if (is_initialized)
    {
        return (RTIXCdrSampleAccessInfo*) &BrakeCommand_g_sampleAccessInfo;
    }
    ...
    BrakeCommand_g_memberAccessInfos[0].bindingMemberValueOffset[0] = 
    (RTIXCdrUnsignedLong) ((char *)&sample->device_id - (char *)sample);

    BrakeCommand_g_memberAccessInfos[1].bindingMemberValueOffset[0] = 
    (RTIXCdrUnsignedLong) ((char *)&sample->brake_intensity - (char *)sample);

    BrakeCommand_g_sampleAccessInfo.memberAccessInfos = 
    BrakeCommand_g_memberAccessInfos;

    {
        size_t candidateTypeSize = sizeof(BrakeCommand);

        if (candidateTypeSize > RTIXCdrUnsignedLong_MAX)
        {
            BrakeCommand_g_sampleAccessInfo.typeSize[0] =
            RTIXCdrUnsignedLong_MAX;
        } 
        else
        {
    ...
    BrakeCommand_g_sampleAccessInfo.languageBinding = 
    RTI_XCDR_TYPE_BINDING_CPP ;
...
RTIXCdrTypePlugin *BrakeCommand_get_type_plugin_info()
{
    static RTIXCdrTypePlugin BrakeCommand_g_typePlugin = 
    {
        NULL, /* serialize */
        NULL, /* serialize_key */
        NULL, /* deserialize_sample */
        ...
        NULL, /* serialized_sample_to_key */
        (RTIXCdrTypePluginInitializeSampleFunction) 
        BrakeCommand_initialize_ex,
        NULL,
        (RTIXCdrTypePluginFinalizeSampleFunction)
        BrakeCommand_finalize_w_return,
        NULL
    };

    return &BrakeCommand_g_typePlugin;
}
#endif
...
RTIBool BrakeCommand_initialize(
    BrakeCommand* sample)
{
    return BrakeCommand_initialize_ex(sample, RTI_TRUE, RTI_TRUE);
}
    ...
    RTIBool allocateMemory)
{
    ...
    return BrakeCommand_initialize_w_params(
        sample,
        &allocParams);
    ...
    const struct DDS_TypeAllocationParams_t * allocParams)
{
    ...
    if (allocParams->allocate_memory) {
        const DDS_Char stringValue[] = "";
        const DDS_String temp = (const DDS_String)stringValue;  
        sample->device_id = DDS_String_alloc(((DEVICE_ID_MAX_LENGTH)));  
        if (!CDR_String_copy(&sample->device_id, &temp, ((DEVICE_ID_MAX_LENGTH))))
        {
            return RTI_FALSE;
        }
    ...
    BrakeCommand* sample)
{
    ...
    BrakeCommand_finalize_ex(sample,RTI_TRUE);
    return RTI_TRUE;
}
...
void BrakeCommand_finalize_ex(
    BrakeCommand* sample,RTIBool deletePointers)
{
    struct DDS_TypeDeallocationParams_t deallocParams =
    DDS_TYPE_DEALLOCATION_PARAMS_DEFAULT;
    ...
    BrakeCommand_finalize_w_params(
        sample,&deallocParams);
}
    ...
    const BrakeCommand* src)
{
    ...
    if (!DDS_String_copy (
        ...
        ((DEVICE_ID_MAX_LENGTH)) + 1)){
        return RTI_FALSE;
    }
    DDS_Primitive_copy (&dst->brake_intensity, &src->brake_intensity);
...
* Defines:  TSeq, T
*
* Configure and implement 'BrakeCommand' sequence class.
*/
#define REDA_SEQUENCE_USER_API
#define T BrakeCommand
#define TSeq BrakeCommandSeq

#define T_initialize BrakeCommand_initialize

#define T_finalize   BrakeCommand_finalize
#define T_copy       BrakeCommand_copy
